...
const KIND_SCALE = {
  ...
  comicshop: 0.85, blakesDesk: 0.56, castlecreative: 0.5,
  // Arcade Island. The hall, the carousel and the arch are landmark-sized and come in at
  // twice a house, so they are scaled to sit beside one; the cabinets, booth, claw machine,
  // skee-ball lane and railing are authored at prop scale already and keep it.
  arcadehall: 0.52, carousel: 0.62, neonarch: 0.7,
  ticketbooth: 1, arcadecabinet: 1, clawmachine: 1, skeeballrun: 1, boardwalkrail: 1,
...
const KINDS = {
  ...
  shield(c, rand) {
    ...
    // The badge face carries the CyberGrade owl, hung on as a decal by the colony (see
    // `_syncBuilding`), so the crest is the real logo rather than a modelled plus.
