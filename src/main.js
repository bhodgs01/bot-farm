...
const actions = {
  ...
  openThread: async () => {
    ...
    try {
      ...
      // Cluster agents open their own dashboard in a tab; harness deep links were launched server-side.
      if (res?.browser && res.url) window.open(res.url, '_blank', 'noopener')
      colony.astronauts.celebrate(thread.id)
      hud.toast(`Opened ${res?.browser ? thread.title : `in ${thread.harnessName || 'your harness'}`}`)
...
const VIEWS = {
  ...
  4: { name: 'Money', zones: ['Active Projects', 'In Process', 'Completed', 'Trade Floor', 'Countdown', 'pay me mother fucker'] },
}
function showView(n) {
  const v = VIEWS[n]
  if (!v) return
