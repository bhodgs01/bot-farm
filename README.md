...
[Bot Crossing](https://github.com/jarrenrocks/bot-crossing) by Jarren Rocks (MIT), pointed at a
Kubernetes cluster instead of a laptop. Every production AI agent is an astronaut, every client is
a hex zone, and every visible state comes from a real signal: pod CPU and running jobs, CrashLoops
and unready deployments, dashboard probes, and the human-facing queues (approvals, escalations).
There is no simulator.

The cluster adapter lives in `server/harnesses/jarvis-cluster.mjs`; the roster at the top of that
file is the whole configuration. Run it against any cluster your `kubectl` can reach with
`npm install && npm run dev`. In-cluster it uses the pod's service account.
